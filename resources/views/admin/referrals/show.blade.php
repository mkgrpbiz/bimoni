...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="px-5 py-3 border-b">
        <h2 class="font-bold text-gray-700">全登録者一覧（{{ $referredUsers->count() }}人）</h2>
    </div>
    <table class="w-full text-sm">
        <thead class="bg-gray-50 text-gray-700">
            <tr>
                ...
                <th class="px-4 py-3 text-left">ユーザーID</th>
                <th class="px-4 py-3 text-left">名前</th>
                <th class="px-4 py-3 text-center">当月報告</th>
                <th class="px-4 py-3 text-right">当月紹介報酬</th>
            </tr>
        </thead>
        <tbody class="divide-y">
            ...
            @php
                ...
                $hasApproved  = $userReports->isNotEmpty();
                $referralPay  = $userReports->sum(fn($r) => $r->campaign?->referral_fee ?? 0);
            @endphp
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3 text-xs text-gray-500">{{ $ru->created_at?->format('Y/m/d') }}</td>
                <td class="px-4 py-3 font-mono text-xs text-pink-600">{{ $ru->referred_by_code }}</td>
                <td class="px-4 py-3 font-mono text-xs text-gray-600">{{ $ru->bimoni_user_id }}</td>
                <td class="px-4 py-3 text-gray-800">{{ $ru->name ?? '（未登録）' }}</td>
                <td class="px-4 py-3 text-center">
                    @if(!$hasApproved)
                        <span class="text-xs text-gray-400">なし</span>
                    @else
                        <span class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded">{{ $userReports->count() }}件</span>
                    @endif
                </td>
                <td class="px-4 py-3 text-right font-medium {{ $referralPay > 0 ? 'text-green-600' : 'text-gray-400' }}">
                    ¥{{ number_format($referralPay) }}
                </td>
            </tr>
            @empty
            <tr>
                <td colspan="6" class="px-4 py-8 text-center text-gray-400">登録者がいません</td>
            </tr>
            @endforelse
        </tbody>
    </table>
