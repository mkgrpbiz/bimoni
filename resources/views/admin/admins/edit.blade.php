...
@section('content')
<div class="max-w-2xl">
    <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4">管理者編集</h1>
    ...
    <form method="POST" action="{{ route('admin.admins.update', $admin) }}"
            ...
            <div class="flex gap-6">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="role" value="operator"
                           {{ old('role', $admin->role) === 'operator' ? 'checked' : '' }}
                           onchange="toggleMenuSelect(this.value)"
                           class="accent-pink-500">
        ...
        <div id="menuSelectArea" class="{{ old('role', $admin->role) === 'operator' ? '' : 'hidden' }}">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-2 border dark:border-gray-600 rounded p-4">
                ...
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="accessible_menus[]" value="{{ $key }}"
                           {{ in_array($key, $currentMenus) ? 'checked' : '' }}
                           class="accent-pink-500">
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit" class="bg-pink-500 text-white px-6 py-2 rounded hover:bg-pink-600 text-sm font-medium">更新</button>
