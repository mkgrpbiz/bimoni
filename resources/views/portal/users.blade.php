...
<form method="GET" class="flex gap-2 mb-4">
    ...
    <input type="text" name="q" value="{{ $q }}"
           class="border rounded px-2 py-1.5 text-sm flex-1" placeholder="ユーザーID/LINE名/氏名/フリガナ">
    <button type="submit" class="bg-gray-800 text-white px-4 py-1.5 rounded text-sm shrink-0">検索</button>
</form>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 text-right font-medium text-blue-600">{{ $collectionCounts->get($user->id, 0) }}</td>
                <td class="px-4 py-3 text-right font-medium text-pink-600">¥{{ number_format($cooperationTotals->get($user->id, 0)) }}</td>
            </tr>
            @empty
    ...
    <div class="bg-white rounded-lg shadow px-4 py-3">
        <div class="flex items-start justify-between mb-1">
            ...
            <div class="text-right shrink-0 ml-2">
                ...
                <p class="text-xs text-gray-400">{{ $user->line_display_name ?? '-' }}</p>
            </div>
        </div>
        <div class="flex gap-3 mt-2 text-xs text-gray-600 border-t pt-2">
            <span>モニター <span class="font-bold text-green-600">{{ $reportCounts->get($user->id, 0) }}</span></span>
            <span>回収 <span class="font-bold text-blue-600">{{ $collectionCounts->get($user->id, 0) }}</span></span>
            <span class="ml-auto">協力金 <span class="font-bold text-pink-600">¥{{ number_format($cooperationTotals->get($user->id, 0)) }}</span></span>
        </div>
    </div>
    @empty
