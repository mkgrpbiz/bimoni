...
<body class="bg-gray-50 min-h-screen pb-24">
    ...
    @auth('liff')
    @if(!request()->routeIs('member.register*') && !request()->routeIs('member.transfer*') && !request()->routeIs('member.faq') && !auth('liff')->user()->transfer_registered_at)
    <style>
    ...
    <div id="line-friend-modal" class="fixed inset-0 z-50 hidden items-center justify-center" style="background: rgba(0,0,0,0.25); backdrop-filter: blur(4px);">
        <div id="line-friend-modal-card" class="bg-white rounded-3xl mx-4 w-full max-w-xs text-center overflow-hidden" style="box-shadow: 0 20px 60px rgba(0,0,0,0.18);">
            ...
            <div class="px-5 pt-4 pb-6">
                ...
                <a href="https://line.me/R/ti/p/{{ config('services.line.official_account_id') }}"
    ...
    <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 shadow-[0_-2px_8px_rgba(0,0,0,0.04)]">
        <div class="flex max-w-2xl mx-auto">
            <a href="{{ route('member.campaigns.index') }}"
               class="flex-1 flex flex-col items-center justify-center gap-1 py-3.5 text-sm font-semibold {{ request()->routeIs('member.campaigns*') ? 'text-pink-500' : 'text-gray-400' }}">
                <span class="flex items-center justify-center w-11 h-8 rounded-full {{ request()->routeIs('member.campaigns*') ? 'bg-pink-50' : '' }}">
            ...
            <a href="{{ route('member.mypage') }}"
               class="flex-1 flex flex-col items-center justify-center gap-1 py-3.5 text-sm font-semibold {{ request()->routeIs('member.mypage') ? 'text-pink-500' : 'text-gray-400' }}">
                <span class="flex items-center justify-center w-11 h-8 rounded-full {{ request()->routeIs('member.mypage') ? 'bg-pink-50' : '' }}">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
