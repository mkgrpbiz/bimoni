...
- `CampaignCourse`（`campaign_courses`テーブル）: `name`, `initial_purchase_fee`, `course_type`（ENUM '単発'/'継続前提'、2026-09-12に'継続'から改称・既存データも一括変換済み）, `continuation_count`（2 or 3、継続前提のみ。フォーム上のラベルは「継続条件」で「2回前提」「3回前提」と表示するが保存値は従来通り数値2/3）, `continuation_judgment_enabled`（bool、2026-09-12追加。継続前提コースで実際に継続確認〈`continuation_response`〉を行い目標継続率を追跡するかどうか）, `continuation_rate`（decimal、2026-09-12追加。このコース内での目標継続率%、`continuation_judgment_enabled=true`の時のみ意味を持つ）, `continuation_fee_2` / `continuation_fee_3`（**`continuation_judgment_enabled=true`の時のみ**設定・表示。無の場合はこの2項目自体を使わない運用）, `percentage`, `invite_message`, `sort_order`
  - `cost()`: 単発=`initial_purchase_fee`のみ、継続前提かつ`continuation_judgment_enabled=true`=`initial_purchase_fee + continuation_fee_2`（3回前提の場合はさらに`+continuation_fee_3`）、継続前提でも判定無の場合は単発と同じ`initial_purchase_fee`のみ
  - **継続判定「有」のコースは応募管理の「目標」欄にこのコース内の目標継続率/実績（`continuation_response='possible'`基準、通常コースの継続率と同じ判定方法）を追加表示し、継続打診ボタンも使えるようになる**（`ApplicationController::campaignIndex()`の`course_stats`、`campaign_index.blade.php`）。判定「無」のコースは従来通り継続打診ボタン非表示・継続率集計対象外のまま
