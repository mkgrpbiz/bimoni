...
- `CampaignCourse`（`campaign_courses`テーブル）: `name`, `initial_purchase_fee`, `course_type`（ENUM '単発'/'継続前提'、2026-09-12に'継続'から改称・既存データも一括変換済み）, `continuation_count`（継続前提は2 or 3、単発+継続判定有は2〜4。フォーム上のラベルは継続前提側が「継続条件」で「2回前提」「3回前提」、単発側が「継続回数」で「2回」〜「4回」、保存値はどちらも数値）, `continuation_fee_2` / `continuation_fee_3` / `continuation_fee_4`（`continuation_fee_4`は2026-09-12追加。継続前提は`continuation_count`に応じてfee_2〈・fee_3〉のみ使用〈fee_4は継続前提では常にnull、上限3回のため〉、単発+継続判定有は`continuation_count`に応じてfee_2〜fee_4を使用）, `continuation_judgment_enabled` / `continuation_rate`（2026-09-12追加、**単発コースのみ**で使う。詳細は下記）, `percentage`, `invite_message`, `sort_order`
  - **「継続前提」と「継続判定の有無」は無関係の別概念**（2026-09-12、一度混同して実装しやり直した）。継続前提＝継続が確定しているコースなので判定不要（回数と各回の購入費のみ設定）。継続判定の有無は**単発コース側**の設定で、単発（本来は1回限りの想定）でも実際には任意で継続が発生しうる場合に、通常コースと同じ`continuation_response`ベースで継続を確認・追跡したい時に「有」にする。「有」の場合のみ「目標継続率」「継続回数」「継続購入費2〜4（継続回数まで）」を追加設定できる（2026-09-12、継続条件有なのに継続購入費が無いと商品代が分からない、という指摘で追加）
  - `cost()`: 単発+継続判定無=`initial_purchase_fee`のみ、継続前提=`initial_purchase_fee + continuation_fee_2`（3回前提の場合はさらに`+continuation_fee_3`、常に無条件で加算＝確定額）、**単発+継続判定有**=`initial_purchase_fee + (継続購入費2〜継続回数までの合計) × (continuation_rate / 100)`（継続が未確定なので目標継続率で期待値化する。外側の加重平均で`percentage`が掛かるのと合わせて「コース利用率×継続率」の複合確率になる。例: 初回1,000円・継続購入費2=500円・目標継続率50%・コース割合30%なら`cost()=1,000+500×0.5=1,250円`、これが加重平均で30%掛かると継続購入費部分には実質15%＝30%×50%が掛かる計算になる。JS側`_form.blade.php`の`calcMonitorCost()`も同じ式で同期させること）
  - **継続判定「有」の単発コースは応募管理の「目標」欄にこのコース内の目標継続率/実績（`continuation_response='possible'`基準、通常コースの継続率と同じ判定方法）を追加表示し、継続打診ボタンも使えるようになる**（`ApplicationController::campaignIndex()`の`course_stats`、`campaign_index.blade.php`。継続前提コースは元々コースが割り当たっている時点で継続打診ボタン非表示のまま、判定という概念がないため対象外）
...
- 詳細ページ（`admin/users/{user}`）: 応募履歴・モニター報告履歴・回収報告履歴・ユーザー招待履歴を`<details>`アコーディオンで表示。**アコーディオンが増えてきたため2026-08-22よりデフォルトは全て閉じた状態に統一**（以前は応募履歴のみopenだったが廃止）
  ...
  - ユーザー招待履歴（2026-08-22追加）: `UserReferralReward.referrer_user_id`がこのユーザーのもの一覧。発生日時 / 招待した相手 / 招待報酬 / 支払いステータス / 支払日。実績サマリーにも「ユーザー招待数」（`$user->referrals()->count()`）「初回実施数」（`referralRewards->count()`）を追加
...
- **「その他」報告と応募の紐付け（2026-09-02追加）**: 「その他」報告（`purchase_type='other'`）は会員側フォームで応募（application_id）を指定せず自由記述で送信されるため、後から管理画面の「案件変更」「報告種別変更」で`initial`に修正しても応募には紐付かないまま。これにより通常フロー（`ReportController::store()`）が使う「同じ応募・同じ種別の非却下報告があれば再報告不可」という重複チェックが素通りし、**実際に本番で6件の二重支払いが発生した**（2026-09-02発覚、合計¥22,635）
  - 対策として「応募と紐付け」機能を追加。対象ユーザーの応募一覧（他の報告が既に紐付いている応募は選択肢から除外）から選んで紐付けると、`campaign_id`・`bonus_amount`（キャンペーン列。会員側の通常フローと同じく応募からコピー）を応募から引き継ぎ、その応募に既に初回/継続の非却下報告があれば紐付け自体を中止する（`ReportController::linkApplication()`）
  - 案件変更・報告種別変更の直後にも、同じユーザー・案件・報告種別の非却下報告が他に無いか確認し、あれば警告バナーを表示する（ハードブロックはしない。応募と紐付けた場合のみ確実にブロックされる）
...
## お友達招待プログラム（2026-08-22〜、2026-08-24に本番リリース）
