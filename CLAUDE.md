...
- `CampaignCourse`（`campaign_courses`テーブル）: `name`, `initial_purchase_fee`, `course_type`（ENUM '単発'/'継続前提'、2026-09-12に'継続'から改称・既存データも一括変換済み）, `continuation_count`（2 or 3、継続前提のみ。フォーム上のラベルは「継続条件」で「2回前提」「3回前提」と表示するが保存値は従来通り数値2/3）, `continuation_fee_2` / `continuation_fee_3`（継続前提のみ、常に設定・表示）, `continuation_judgment_enabled` / `continuation_rate`（2026-09-12追加、**単発コースのみ**で使う。詳細は下記）, `percentage`, `invite_message`, `sort_order`
  - **「継続前提」と「継続判定の有無」は無関係の別概念**（2026-09-12、一度混同して実装しやり直した）。継続前提＝継続が確定しているコースなので判定不要（回数と各回の購入費のみ設定）。継続判定の有無は**単発コース側**の設定で、単発（本来は1回限りの想定）でも実際には任意で継続が発生しうる場合に、通常コースと同じ`continuation_response`ベースで継続を確認・追跡したい時に「有」にする。「有」の場合のみ「目標継続率」（このコース内での目標%）を追加設定できる
  - `cost()`: 単発（継続判定の有無に関わらず）=`initial_purchase_fee`のみ、継続前提=`initial_purchase_fee + continuation_fee_2`（3回前提の場合はさらに`+continuation_fee_3`）
